...
const getAddress = async (park) => {
  ...
  .then((r) => {
    ...
    candidates.forEach((candidate) =>
      candidate.address_components.forEach((address) => {
...
.then((res) => {
  ...
  usParks = allParks.filter((park) => park.textContent.includes('U.S.')).map((park) => { return {name: park.children[0].textContent, webPage: park.children[0].href} });
